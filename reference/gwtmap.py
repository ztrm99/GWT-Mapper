...
def present_methods(methods, quiet, send_probe):
    ...
    for _, method in enumerate(methods):
        ...
        service_label = method["serviceProxy"]
        if service_label.endswith("_Proxy"):
            service_label = service_label[:-6]
        elif service_label.endswith("Proxy"):
            service_label = service_label[:-5]
        ...
        if FILTER in method_string:
            if service_label != service_category:
                writer(f"\n----- {service_label} -----\n")
                ret_service_count += 1
...
##################################################
# Modern (GWT 2.9+) method extraction helpers
##################################################
def build_string_var_map(code):
    """ Returns a map of obfuscated var name -> string literal """
    string_vars = {}
    joined = "\n".join(code)
...
def detect_rpc_invoke_func(code):
    """ Attempts to identify the obfuscated RPC invoke function """
    ...
    # Fallback: look for calls that include a literal interface name
    invoke_candidates = {}
    ...
    if invoke_candidates:
        return max(invoke_candidates, key=invoke_candidates.get)

    return None

def resolve_string(value, string_vars):
    """ Resolves a variable to its string literal, if available """
    if value in string_vars:
        return string_vars[value]
    return value

def extract_method_info_modern(code, string_vars):
    """ Returns a List of enumerated GWT method objects (modern GWT) """
    invoke_fn = detect_rpc_invoke_func(code) or "gVb"
    new_pattern = re.compile(
        r"new\s+[A-Za-z0-9_\.\$]+\([^;]*,\s*([A-Za-z0-9_\$]+)\s*,\s*'([^']+)'\s*\)"
    )
    invoke_pattern = re.compile(
        rf"{re.escape(invoke_fn)}\([^;]*,\s*([A-Za-z0-9_\$]+|'[^']+')\s*,\s*([0-9]+)\)"
    )
    ...
    for i, line in enumerate(code):
        ...
        if not m:
            continue
        service_proxy_var, method_name = m.group(1), m.group(2)
        iface_val, param_count = None, None

        for j in range(i + 1, min(i + 25, len(code))):
            sm = invoke_pattern.search(code[j])
            if sm:
                iface_val = sm.group(1)
        ...
        iface_val = iface_val.strip("'")
        iface_val = resolve_string(iface_val, string_vars)
        if "." not in iface_val:
            continue

        service_proxy = resolve_string(service_proxy_var, string_vars)
        try:
            param_count_int = int(param_count)
        except ValueError:
            param_count_int = 0
...
def main():
    ...
    if len(method_objects) == 0:
        string_vars = build_string_var_map(code)
        method_objects = extract_method_info_modern(code, string_vars)

    if len(service_objects) == 0 and len(method_objects) != 0:
        svc_set = []
        for m in method_objects:
            if m["rmtSvcIntName"] not in svc_set:
                svc_set.append(m["rmtSvcIntName"])
        service_objects = [
            {"servicePath": svc, "strongName": UNKNOWN}
            for svc in svc_set
        ]
